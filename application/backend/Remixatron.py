...
def symmetrize_matrix(matrix):
    ...
    Args:
        matrix: A square matrix to be symmetrized
    ...
    if matrix.shape[0] != matrix.shape[1]:
        raise ValueError(f"Matrix is not square: shape is {matrix.shape}")
        
    try:
        # Create a copy to avoid modifying the original
        matrix_symm = matrix.copy()

      # Get the indices of the upper triangle (excluding diagonal)
        i_upper = np.triu_indices_from(matrix_symm, k=1)
        
        # Calculate the average of corresponding elements
        avg_values = 0.5 * (matrix_symm[i_upper] + matrix_symm[i_upper[1], i_upper[0]])
        
        # Set both upper and lower triangular elements to the average
        matrix_symm[i_upper] = avg_values
        matrix_symm[i_upper[1], i_upper[0]] = avg_values
 
        return matrix_symm
    except Exception as e:
        print(f"Error in symmetrize_matrix: {e}")
        # Return the original matrix if symmetrization fails
        return matrix
...
class InfiniteJukebox(object):
    ...
    def __init__(self, filename, start_beat=1, clusters=0, progress_callback=None,
       ...
       progress_callback: a callback function that will get periodic satatus updates as
        ...
        # Initialize _starting_beat_cache as an empty numpy array if None is provided
        if starting_beat_cache is None:
            self._starting_beat_cache = np.array([])
        else:
    ...
    def __process_audio(self):
        ...
        if self._starting_beat_cache.size == 0:
            ...
            try:
                ...
                print(f"Warning: Madmom beat detection failed: {e}. Falling back to librosa.")
                
                # Fallback to librosa's beat detection
                self.__report_progress( .3, "Falling back to librosa beat detection..." )
                
                tempo, beat_frames = librosa.beat.beat_track(y=y, sr=sr, trim=False)
                beat_times = librosa.frames_to_time(beat_frames, sr=sr)
                
                # Convert to the format expected by the rest of the code
                # Create a 2D array with beat times and a default bar position of 1
                downbeats = np.column_stack((beat_times, np.ones(len(beat_times))))
